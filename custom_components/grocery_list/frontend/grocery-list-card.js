...
`,_t={add_placeholder:"Add an item…",add:"Add",qty:"Qty",unit:"Unit",category:"Category",uncategorized:"Uncategorized",clear_checked:"Clear checked",undo:"Undo",redo:"Redo",sync:"Sync",save:"Save",cancel:"Cancel",delete:"Delete",edit:"Edit",empty_list:"Nothing here yet. Add your first item above.",sync_synced:"Synced",sync_pending:"Pending",sync_syncing:"Syncing…",sync_offline:"Offline",sync_error:"Sync error",manage_categories:"Manage categories",categories:"Categories",new_category:"New category",category_name_en:"Name (English)",category_name_de:"Name (German)",add_category:"Add category",no_categories:"No categories yet. Create one above.",move_up:"Move up",move_down:"Move down",close:"Close",delete_category_confirm:"Delete this category? Its items become uncategorized.",archive:"Archive",view_archive:"View archive",no_archive:"Nothing archived yet. Cleared items appear here.",archived_on:"Archived",needs_config:"Grocery List not configured",needs_config_hint:"Open the card editor and pick a Grocery List, or set 'entry_id' in YAML.",select_list_entry:"Grocery List instance",no_entries:"No Grocery List integration found. Add it under Settings → Devices & Services first.",title:"Title (optional)",manage_lists:"Manage lists",lists:"Lists",new_list:"New list",list_name:"List name",add_list:"Add list",no_lists:"No lists yet. Create one above.",rename_list:"Rename list",delete_list:"Delete list",delete_list_confirm:"Delete this list and all its items? This cannot be undone by others once synced.",list_name_prompt:"New list name:"},mt={en:_t,de:{add_placeholder:"Artikel hinzufügen…",add:"Hinzufügen",qty:"Menge",unit:"Einheit",category:"Kategorie",uncategorized:"Ohne Kategorie",clear_checked:"Erledigte entfernen",undo:"Rückgängig",redo:"Wiederholen",sync:"Sync",save:"Speichern",cancel:"Abbrechen",delete:"Löschen",edit:"Bearbeiten",empty_list:"Noch nichts hier. Füge oben deinen ersten Artikel hinzu.",sync_synced:"Synchronisiert",sync_pending:"Ausstehend",sync_syncing:"Synchronisiere…",sync_offline:"Offline",sync_error:"Sync-Fehler",manage_categories:"Kategorien verwalten",categories:"Kategorien",new_category:"Neue Kategorie",category_name_en:"Name (Englisch)",category_name_de:"Name (Deutsch)",add_category:"Kategorie hinzufügen",no_categories:"Noch keine Kategorien. Erstelle oben eine.",move_up:"Nach oben",move_down:"Nach unten",close:"Schließen",delete_category_confirm:"Diese Kategorie löschen? Ihre Artikel werden dann ohne Kategorie angezeigt.",archive:"Archiv",view_archive:"Archiv anzeigen",no_archive:"Noch nichts archiviert. Entfernte Artikel erscheinen hier.",archived_on:"Archiviert",needs_config:"Einkaufsliste nicht konfiguriert",needs_config_hint:"Öffne den Karten-Editor und wähle eine Einkaufsliste, oder setze 'entry_id' im YAML.",select_list_entry:"Einkaufslisten-Instanz",no_entries:"Keine Einkaufslisten-Integration gefunden. Füge sie zuerst unter Einstellungen → Geräte & Dienste hinzu.",title:"Titel (optional)",manage_lists:"Listen verwalten",lists:"Listen",new_list:"Neue Liste",list_name:"Listenname",add_list:"Liste hinzufügen",no_lists:"Noch keine Listen. Erstelle oben eine.",rename_list:"Liste umbenennen",delete_list:"Liste löschen",delete_list_confirm:"Diese Liste und alle ihre Artikel löschen? Nach der Synchronisierung können andere dies nicht rückgängig machen.",list_name_prompt:"Name der neuen Liste:"}};function vt(t){return t&&t.toLowerCase().startsWith("de")?"de":"en"}function yt(t){const e=mt[t]??_t;return t=>e[t]??_t[t]??t}const ft="__none__";let bt=class extends at{constructor(){super(...arguments),this._units=[],this._defaultUnit="pcs",this._editingId=null,this._editValue="",this._draftName="",this._draftQty=1,this._draftUnit="",this._draftCategory=null,this._catManagerOpen=!1,this._archiveOpen=!1,this._newCatEn="",this._newCatDe="",this._listManagerOpen=!1,this._newListName=""}setConfig(t){const e=t??{type:"custom:grocery-list-card",entry_id:""};this._config=e,e.slug&&(this._activeSlug=e.slug),this._subscribedEntry&&this._subscribedEntry!==e.entry_id&&this._teardown(),this._maybeSubscribe()}static getConfigElement(){return document.createElement("grocery-list-card-editor")}static async getStubConfig(t){let e="";try{const i=(await t.connection.sendMessagePromise({type:"config_entries/get",domain:"grocery_list"})).find(t=>"grocery_list"===t.domain);i&&(e=i.entry_id)}catch(t){}return{type:"custom:grocery-list-card",entry_id:e}}getCardSize(){const t=this._activeList()?.items.length??0;return 2+Math.ceil(t/3)}connectedCallback(){super.connectedCallback(),this._maybeSubscribe()}disconnectedCallback(){super.disconnectedCallback(),this._teardown()}updated(t){t.has("hass")&&this._maybeSubscribe()}get _lang(){return vt(this.hass?.locale?.language??this.hass?.language)}async _maybeSubscribe(){if(this.hass&&this._config&&this._config.entry_id&&this._subscribedEntry!==this._config.entry_id){this._teardown(),this._subscribedEntry=this._config.entry_id,this._api=new ut(this.hass,this._config.entry_id);try{const t=await this._api.getUnits();this._units=t.units,this._defaultUnit=t.default_unit,this._draftUnit||(this._draftUnit=t.default_unit)}catch(t){this._units=[]}this._unsub=await this._api.subscribe(t=>{this._snapshot=t,!this._activeSlug&&t.lists.length&&(this._activeSlug=t.lists[0].slug)},this._lang)}}_teardown(){this._unsub&&(this._unsub(),this._unsub=void 0),this._subscribedEntry=void 0}_activeList(){const t=this._snapshot;if(t)return t.lists.find(t=>t.slug===this._activeSlug)??t.lists[0]}_targetSlug(){return this._activeList()?.slug??this._activeSlug??this._config?.slug??"default"}render(){const t=yt(this._lang);if(!this._config?.entry_id)return q`<ha-card>
    ...
    `}_fmtArchiveTs(t){const e=new Date(t);return isNaN(e.getTime())?t:e.toLocaleDateString(this._lang,{year:"numeric",month:"short",day:"numeric"})}_renderListManager(t){const e=this._snapshot?.lists??[];return q`
      <div
        class="gl-overlay"
      ...
      >
        <div class="gl-sheet">
          ...
          ${e.length?q`<ul class="gl-catlist">
              ...
              </ul>`:q`<div class="gl-empty">${t("no_lists")}</div>`}
    ...
    `}_renderCategoryManager(t){const e=this._categories();return q`
      <div
        class="gl-overlay"
    ...
    `}_categories(){return this._snapshot?.categories??[]}_catLabel(t){return t.labels[this._lang]??t.labels.en??t.id}_unitLabel(t){const e=this._units.find(e=>e.id===t);return e?e.labels[this._lang]??e.labels.en??t:t}_fmtNum(t){return Number.isInteger(t)?String(t):t.toFixed(2).replace(/0+$/,"")}_bumpQty(t){this._draftQty=Math.max(0,Math.round(100*(this._draftQty+t))/100)}_beginEdit(t){this._editingId=t.id,this._editValue=t.name}_cancelEdit(){this._editingId=null,this._editValue=""}_saveEdit(t,e){const i=this._editValue.trim();i&&i!==e.name&&this._api?.updateItem(t,e.id,{name:i}),this._cancelEdit()}_commitAdd(){const t=this._draftName.trim();if(!t||!this._api)return;const e=this._targetSlug();this._api.addItem(e,t,{category:this._draftCategory,qty_value:this._draftQty||null,qty_unit:this._draftQty?this._draftUnit||this._defaultUnit:null}),this._activeSlug=e,this._draftName=""}_closeCatManager(){this._catManagerOpen=!1,this._newCatEn="",this._newCatDe=""}_commitNewCategory(){const t=this._newCatEn.trim(),e=this._newCatDe.trim();if(!t&&!e)return;const i={};t&&(i.en=t),e&&(i.de=e),i.en||(i.en=e),this._api?.createCategory(i),this._newCatEn="",this._newCatDe=""}_renameCategory(t,e,i){const s=i.trim();if(s===(t.labels[e]??""))return;const r={...t.labels};s?r[e]=s:delete r[e],r.en||(r.en=s||r.de||t.id),this._api?.updateCategory(t.id,{labels:r})}_moveCategory(t,e){const i=this._categories().map(t=>t.id),s=t+e;s<0||s>=i.length||([i[t],i[s]]=[i[s],i[t]],this._api?.reorderCategories(i))}_deleteCategory(t,e){window.confirm(e("delete_category_confirm"))&&this._api?.deleteCategory(t.id)}_closeListManager(){this._listManagerOpen=!1,this._newListName=""}async _commitNewList(){const t=this._newListName.trim();if(t&&this._api){this._newListName="";try{const e=await this._api.createList(t);e?.list?.slug&&(this._activeSlug=e.list.slug)}catch(t){}}}_renameListPrompt(t,e){const i=window.prompt(e("list_name_prompt"),t.title);if(null===i)return;const s=i.trim();s&&s!==t.title&&this._api?.renameList(t.slug,s)}_deleteListConfirm(t,e){if(window.confirm(e("delete_list_confirm"))&&(this._api?.deleteList(t.slug),this._activeSlug===t.slug)){const e=(this._snapshot?.lists??[]).filter(e=>e.slug!==t.slug);this._activeSlug=e[0]?.slug}}};bt.styles=pt,t([ht({attribute:!1})],bt.prototype,"hass",void 0),t([gt()],bt.prototype,"_config",void 0),t([gt()],bt.prototype,"_snapshot",void 0),t([gt()],bt.prototype,"_units",void 0),t([gt()],bt.prototype,"_defaultUnit",void 0),t([gt()],bt.prototype,"_activeSlug",void 0),t([gt()],bt.prototype,"_editingId",void 0),t([gt()],bt.prototype,"_editValue",void 0),t([gt()],bt.prototype,"_draftName",void 0),t([gt()],bt.prototype,"_draftQty",void 0),t([gt()],bt.prototype,"_draftUnit",void 0),t([gt()],bt.prototype,"_draftCategory",void 0),t([gt()],bt.prototype,"_catManagerOpen",void 0),t([gt()],bt.prototype,"_archiveOpen",void 0),t([gt()],bt.prototype,"_newCatEn",void 0),t([gt()],bt.prototype,"_newCatDe",void 0),t([gt()],bt.prototype,"_listManagerOpen",void 0),t([gt()],bt.prototype,"_newListName",void 0),bt=t([lt("grocery-list-card")],bt);let $t=class extends at{constructor(){super(...arguments),this._config={type:"custom:grocery-list-card",entry_id:""},this._entries=[]}setConfig(t){this._config=t??{type:"custom:grocery-list-card",entry_id:""}}connectedCallback(){super.connectedCallback(),this._loadEntries()}async _loadEntries(){if(this.hass)try{const t=await this.hass.connection.sendMessagePromise({type:"config_entries/get",domain:"grocery_list"});this._entries=t.filter(t=>"grocery_list"===t.domain).map(t=>({entry_id:t.entry_id,title:t.title}))}catch(t){this._entries=[]}}get _lang(){return vt(this.hass?.locale?.language??this.hass?.language)}_emit(t){this._config=t,this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:t},bubbles:!0,composed:!0}))}render(){const t=yt(this._lang);return q`
      <div class="gl-editor">
        ...
        ${this._entries.length?q`<select
            ...
            >
              ...
              ${this._entries.map(t=>q`<option
                  value=${t.entry_id}
                  ?selected=${t.entry_id===this._config.entry_id}
                >
                  ${t.title||t.entry_id}
                </option>`)}
            </select>`:q`<p>${t("no_entries")}</p>`}
        <label>${t("title")}</label>
        <input
