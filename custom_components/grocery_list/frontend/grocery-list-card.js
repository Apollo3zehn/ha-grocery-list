...
`,_t={add_placeholder:"Add an item…",add:"Add",qty:"Qty",unit:"Unit",category:"Category",uncategorized:"Uncategorized",clear_checked:"Clear checked",undo:"Undo",redo:"Redo",sync:"Sync",save:"Save",cancel:"Cancel",delete:"Delete",edit:"Edit",empty_list:"Nothing here yet. Add your first item above.",sync_synced:"Synced",sync_pending:"Pending",sync_syncing:"Syncing…",sync_offline:"Offline",sync_error:"Sync error",manage_categories:"Manage categories",categories:"Categories",new_category:"New category",category_name_en:"Name (English)",category_name_de:"Name (German)",add_category:"Add category",no_categories:"No categories yet. Create one above.",move_up:"Move up",move_down:"Move down",close:"Close",delete_category_confirm:"Delete this category? Its items become uncategorized.",archive:"Archive",view_archive:"View archive",no_archive:"Nothing archived yet. Cleared items appear here.",archived_on:"Archived",needs_config:"Grocery List not configured",needs_config_hint:"Open the card editor and pick a Grocery List, or set 'entry_id' in YAML.",select_list_entry:"Grocery List instance",no_entries:"No Grocery List integration found. Add it under Settings → Devices & Services first.",title:"Title (optional)"},vt={en:_t,de:{add_placeholder:"Artikel hinzufügen…",add:"Hinzufügen",qty:"Menge",unit:"Einheit",category:"Kategorie",uncategorized:"Ohne Kategorie",clear_checked:"Erledigte entfernen",undo:"Rückgängig",redo:"Wiederholen",sync:"Sync",save:"Speichern",cancel:"Abbrechen",delete:"Löschen",edit:"Bearbeiten",empty_list:"Noch nichts hier. Füge oben deinen ersten Artikel hinzu.",sync_synced:"Synchronisiert",sync_pending:"Ausstehend",sync_syncing:"Synchronisiere…",sync_offline:"Offline",sync_error:"Sync-Fehler",manage_categories:"Kategorien verwalten",categories:"Kategorien",new_category:"Neue Kategorie",category_name_en:"Name (Englisch)",category_name_de:"Name (Deutsch)",add_category:"Kategorie hinzufügen",no_categories:"Noch keine Kategorien. Erstelle oben eine.",move_up:"Nach oben",move_down:"Nach unten",close:"Schließen",delete_category_confirm:"Diese Kategorie löschen? Ihre Artikel werden dann ohne Kategorie angezeigt.",archive:"Archiv",view_archive:"Archiv anzeigen",no_archive:"Noch nichts archiviert. Entfernte Artikel erscheinen hier.",archived_on:"Archiviert",needs_config:"Einkaufsliste nicht konfiguriert",needs_config_hint:"Öffne den Karten-Editor und wähle eine Einkaufsliste, oder setze 'entry_id' im YAML.",select_list_entry:"Einkaufslisten-Instanz",no_entries:"Keine Einkaufslisten-Integration gefunden. Füge sie zuerst unter Einstellungen → Geräte & Dienste hinzu.",title:"Titel (optional)"}};function yt(t){return t&&t.toLowerCase().startsWith("de")?"de":"en"}function ft(t){const e=vt[t]??_t;return t=>e[t]??_t[t]??t}const mt="__none__";let bt=class extends at{constructor(){super(...arguments),this._units=[],this._defaultUnit="pcs",this._editingId=null,this._editValue="",this._draftName="",this._draftQty=1,this._draftUnit="",this._draftCategory=null,this._catManagerOpen=!1,this._archiveOpen=!1,this._newCatEn="",this._newCatDe=""}setConfig(t){const e=t??{type:"custom:grocery-list-card",entry_id:""};this._config=e,e.slug&&(this._activeSlug=e.slug),this._subscribedEntry&&this._subscribedEntry!==e.entry_id&&this._teardown(),this._maybeSubscribe()}static getConfigElement(){return document.createElement("grocery-list-card-editor")}static async getStubConfig(t){let e="";try{const i=(await t.connection.sendMessagePromise({type:"config_entries/get",domain:"grocery_list"})).find(t=>"grocery_list"===t.domain);i&&(e=i.entry_id)}catch(t){}return{type:"custom:grocery-list-card",entry_id:e}}getCardSize(){const t=this._activeList()?.items.length??0;return 2+Math.ceil(t/3)}connectedCallback(){super.connectedCallback(),this._maybeSubscribe()}disconnectedCallback(){super.disconnectedCallback(),this._teardown()}updated(t){t.has("hass")&&this._maybeSubscribe()}get _lang(){return yt(this.hass?.locale?.language??this.hass?.language)}async _maybeSubscribe(){if(this.hass&&this._config&&this._config.entry_id&&this._subscribedEntry!==this._config.entry_id){this._teardown(),this._subscribedEntry=this._config.entry_id,this._api=new ut(this.hass,this._config.entry_id);try{const t=await this._api.getUnits();this._units=t.units,this._defaultUnit=t.default_unit,this._draftUnit||(this._draftUnit=t.default_unit)}catch(t){this._units=[]}this._unsub=await this._api.subscribe(t=>{this._snapshot=t,!this._activeSlug&&t.lists.length&&(this._activeSlug=t.lists[0].slug)},this._lang)}}_teardown(){this._unsub&&(this._unsub(),this._unsub=void 0),this._subscribedEntry=void 0}_activeList(){const t=this._snapshot;if(t)return t.lists.find(t=>t.slug===this._activeSlug)??t.lists[0]}_targetSlug(){return this._activeList()?.slug??this._activeSlug??this._config?.slug??"default"}render(){const t=ft(this._lang);if(!this._config?.entry_id)return q`<ha-card>
    ...
    `}_renderAddBar(t){return q`
      ...
      <div class="gl-qtyrow">
        ...
        <select
          class="gl-cat"
          .value=${this._draftCategory??mt}
          @change=${t=>{const e=t.target.value;this._draftCategory=e===mt?null:e}}
        >
          <option value=${mt}>${t("uncategorized")}</option>
          ${this._categories().map(t=>q`<option value=${t.id}>${this._catLabel(t)}</option>`)}
        </select>
      </div>
    `}_renderGroups(t,e){if(!t.items.length)return q`<div class="gl-empty">${e("empty_list")}</div>`;const i=this._categories(),s=new Map;i.forEach((t,e)=>s.set(t.id,e));const r=new Map;for(const e of t.items){const t=e.category??mt,i=r.get(t);i?i.push(e):r.set(t,[e])}const n=[...r.keys()].sort((t,e)=>t===mt?1:e===mt?-1:(s.get(t)??999)-(s.get(e)??999));return q`
      ${n.map(i=>{const s=this._sortSunk(r.get(i)),n=i===mt?e("uncategorized"):this._snapshot.category_labels[i]??i;return q`
          <div class="gl-group">
